...
class UploadFileForm(forms.Form):
    ...
    def clean_file(self):
        ...
        if not name.endswith(ALLOWED_EXTENSIONS):
            raise forms.ValidationError("Неподдерживаемый тип файла. Разрешены: CSV, XLSX, JSON.")
...
def make_column_mapping_form(choices):
    ...
    class ColumnMappingForm(forms.Form):
        review_text = forms.ChoiceField(label="Столбец с текстом отзыва", choices=choices)  # ОБЯЗАТЕЛЬНО
        ...
        product_category = forms.ChoiceField(label="Категория товара", required=False, choices=col_choices)
        ...
        source = forms.ChoiceField(label="Источник", required=False, choices=col_choices)
